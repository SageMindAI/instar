...
describe('Trigger mode (mention vs all)', () => {
  let tmpDir: string;
  ...
  it('can be set to all mode', () => {
    const adapter = new IMessageAdapter(
      { authorizedContacts: ['+14081234567'], triggerMode: 'all' } as any,
      tmpDir,
    );
    expect(adapter.getTriggerMode()).toBe('all');
  });

  it('in "all" mode, every message triggers', () => {
    const adapter = new IMessageAdapter(
      { authorizedContacts: ['+14081234567'], triggerMode: 'all' } as any,
      tmpDir,
    );
    const result = (adapter as any)._checkTrigger('hello there');
    expect(result.triggered).toBe(true);
    expect(result.strippedText).toBe('hello there');
  });

  it('in "mention" mode, message without mention does NOT trigger', () => {
    const adapter = new IMessageAdapter(
      { authorizedContacts: ['+14081234567'], triggerMode: 'mention' } as any,
      tmpDir,
    );
  ...
  it('in "mention" mode, message with @AgentName triggers', () => {
    const adapter = new IMessageAdapter(
      { authorizedContacts: ['+14081234567'], triggerMode: 'mention' } as any,
      tmpDir,
    );
    adapter.setAgentName('Echo');
    const result = (adapter as any)._checkTrigger('@Echo what time is it?');
    expect(result.triggered).toBe(true);
    expect(result.strippedText).toBe('what time is it?');
  });
  ...
  it('mention detection is case-insensitive', () => {
    ...
    expect((adapter as any)._checkTrigger('@echo do something').triggered).toBe(true);
    expect((adapter as any)._checkTrigger('@ECHO do something').triggered).toBe(true);
    expect((adapter as any)._checkTrigger('@Echo do something').triggered).toBe(true);
  });

  it('strips mention from message content', () => {
    const adapter = new IMessageAdapter(
      { authorizedContacts: ['+14081234567'], triggerMode: 'mention' } as any,
      tmpDir,
    );
    ...
    // Leading mention
    expect((adapter as any)._checkTrigger('@Echo check the weather').strippedText).toBe('check the weather');
    // Mid-sentence mention
    expect((adapter as any)._checkTrigger('hey @Echo can you help?').strippedText).toBe('hey can you help?');
    // Trailing mention
    expect((adapter as any)._checkTrigger('what do you think @Echo').strippedText).toBe('what do you think');
  });

  it('mention mode with no agent name falls back to all mode', () => {
    const adapter = new IMessageAdapter(
      { authorizedContacts: ['+14081234567'], triggerMode: 'mention' } as any,
      tmpDir,
    );
    // No setAgentName called, agentName is empty string
    const result = (adapter as any)._checkTrigger('hello there');
    expect(result.triggered).toBe(true);
  });
});

...
export class IMessageAdapter implements MessagingAdapter {
  ...
  constructor(config: Record<string, unknown>, stateDir: string) {
    ...
    this.agentName = (this.config.agentName ?? '').toLowerCase();
  ...
  private async _handleIncomingMessage(msg: IMessageIncoming): Promise<void> {
    ...
    // Trigger mode check — in "mention" mode, only route messages that @mention the agent
    const triggerResult = this._checkTrigger(msg.text);
    if (!triggerResult.triggered) {
      // Message logged and events emitted above, but agent stays silent
      return;
    }

    // Route to registered message handler (with mention stripped from content)
    if (this.messageHandler) {
      const message: Message = {
        id: msg.messageId,
        userId: msg.sender,
        content: triggerResult.strippedText,
  ...
  /**
   * Check if a message triggers the agent based on triggerMode.
   *
   * - "all": every message triggers (original behavior)
   * - "mention": only messages containing @{agentName} trigger
   *
   * Returns the stripped text (mention removed) when triggered.
   */
  _checkTrigger(text: string): { triggered: boolean; strippedText: string } {
    ...
    // "mention" mode — look for @agentName (case-insensitive)
    if (!this.agentName) {
      // No agent name configured — fall back to "all" mode
      return { triggered: true, strippedText: text };
    }

    const mentionPattern = new RegExp(`@${this.agentName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i');
  ...
  /** Set the agent name for mention detection (called by server.ts with project name). */
  setAgentName(name: string): void {
    (this as any).agentName = name.toLowerCase();
  }

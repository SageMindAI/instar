  ...
  if (options.foreground) {
    ...
    if (imessageConfig) {
      try {
        ...
        // Set agent name for mention-based triggering (defaults to project name)
        const agentName = (imessageConfig.config as any)?.agentName || config.projectName;
        if (agentName) imessageAdapter.setAgentName(agentName);

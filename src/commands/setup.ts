 ...
 *
 * The wrapper checks for shadow install first, falls back to global binary.
...
# Resolves the shadow install (auto-updated version) at startup, falling back
# to the global binary. Ensures machine reboots use the latest version.
SHADOW="${shadowCli}"
if [ -f "$SHADOW" ]; then
  exec node "$SHADOW" "$@"
else

...
const VALID_SORTS = ['significance', 'recent', 'name'];
export function createRoutes(ctx) {
    ...
    router.post('/hooks/events', (req, res) => {
        ...
        if (ctx.subagentTracker && payload.session_id) {
            if (payload.event === 'SubagentStart' && payload.agent_id && payload.agent_type) {
                ...
                ctx.subagentTracker.onStop(payload.agent_id, payload.session_id, payload.last_assistant_message, payload.agent_transcript_path);
            }
        }
    ...
    router.post('/agents/:name/restart', async (req, res) => {
        ...
        try {
            const { listAgents } = await import('../core/AgentRegistry.js');
            const agents = listAgents();
            const agent = agents.find((a) => a.name === name || a.name === `${name}-lifeline`);
            if (!agent) {
    ...
    router.post('/backups/:id/restore', async (req, res) => {
        ...
        try {
            const { BackupManager } = await import('../core/BackupManager.js');
            const manager = new BackupManager(ctx.config.stateDir, undefined, () => ctx.sessionManager.listRunningSessions().length > 0);
    ...
    router.post('/semantic/remember', (req, res) => {
        ...
        if (!type || !name || !content || confidence === undefined || !source) {
            res.status(400).json({ error: 'Missing required fields: type, name, content, confidence, source' });
            return;
        }
    ...
    router.post('/semantic/connect', (req, res) => {
        ...
        if (!fromId || !toId || !relation) {
            res.status(400).json({ error: 'Missing required fields: fromId, toId, relation' });
            return;
        }
        if (!VALID_RELATION_TYPES.has(relation)) {
            res.status(400).json({ error: `Invalid relation type: ${relation}` });
            return;
        }
        try {
    ...
    router.get('/semantic/search', (req, res) => {
        ...
        try {
            ...
            const results = ctx.semanticMemory.search(query, { types: types, domain, minConfidence, limit });
    ...
    router.get('/semantic/search/hybrid', async (req, res) => {
        ...
        try {
            ...
            const domain = req.query.domain;
            const minConfidence = req.query.minConfidence ? parseFloat(req.query.minConfidence) : undefined;
            const results = await ctx.semanticMemory.searchHybrid(query, { types: types, domain, minConfidence, limit });
    ...
    router.get('/semantic/explore/:id', (req, res) => {
        ...
        try {
            const maxDepth = parseInt(req.query.maxDepth, 10) || 2;
            const relations = req.query.relations ? String(req.query.relations).split(',') : undefined;
            const results = ctx.semanticMemory.explore(req.params.id, { maxDepth, relations: relations });
            res.json({ results });
        }
        catch (err) {
    ...
    router.post('/semantic/supersede', (req, res) => {
        ...
        if (!oldId || !newId) {
            res.status(400).json({ error: 'Missing required fields: oldId, newId' });
            return;
        }
        try {
    ...
    router.get('/semantic/stale', (req, res) => {
        ...
        try {
            const maxConfidence = req.query.maxConfidence ? parseFloat(req.query.maxConfidence) : undefined;
    ...
    router.get('/semantic/context', (req, res) => {
        ...
        try {
            const query = String(req.query.q || '');
            const maxTokens = parseInt(req.query.maxTokens, 10) || 2000;
    ...
    router.post('/semantic/export-memory', async (req, res) => {
        ...
        try {
            ...
            if (filePath) {
                ...
                try {
                    ctx.semanticMemory.writeSnapshot();
                }
                catch { /* non-critical */ }
                res.json(result);
            }
    ...
    router.get('/capabilities', (_req, res) => {
        ...
        res.json({
            ...
            git: (() => {
                ...
                if (ctx.scheduler) {
                    const gitJob = ctx.scheduler.getJobs().find((j) => j.slug === 'git-sync');
                    gitSyncJobEnabled = !!gitJob?.enabled;
                }
                return {
                    inRepo: hasGitRepo,
                    hasRemote,
    ...
    router.get('/self-knowledge/search', async (req, res) => {
        ...
        try {
            ...
            const maxBudget = req.query.maxTokens ? parseInt(req.query.maxTokens, 10) : undefined;
            if (dryRun) {
    ...
    router.post('/coherence/check', (req, res) => {
        ...
        const result = ctx.coherenceGate.check(action, context);
        res.json(result);
    });
    router.post('/coherence/reflect', (req, res) => {
        ...
        const prompt = ctx.coherenceGate.generateReflectionPrompt(action, context);
    ...
    router.get('/context/active-job', (_req, res) => {
        const activeJob = ctx.state.get('active-job');
        if (!activeJob) {
        ...
        const enriched = sessions.map(s => {
            ...
            if (!result.platform && ctx.slack) {
                ...
                if (channelId) {
                    ...
                    if (registry[channelId]?.channelName)
                        result.platformName = registry[channelId].channelName;
                }
            }
            if (!result.platform) {
    ...
    router.get('/sessions/:name/output', (req, res) => {
        ...
        const rawLines = parseInt(req.query.lines, 10) || 100;
    ...
    router.post('/sessions/create', spawnLimiter, async (req, res) => {
        ...
        if (resolvedPlatform === 'slack' && ctx.slack) {
            try {
                const rawAgentName = ctx.slack.config?.workspaceName?.replace(/-agent$/i, '') || 'agent';
        ...
        try {
            ...
            if (slackChannelId && ctx.slack) {
                ...
                try {
                    const slackConfig = ctx.config.messaging?.find(m => m.type === 'slack')?.config;
                    const authorizedUserIds = slackConfig?.authorizedUserIds ?? [];
    ...
    router.get('/jobs/category-report/:category', (req, res) => {
        ...
        const matchingJobs = allJobs.filter(j => j.tags?.includes(categoryTag) || j.tags?.includes(category));
    ...
    router.post('/jobs/:slug/trigger', (req, res) => {
        ...
        const rawReason = req.body?.reason || 'manual';
    ...
    const manualTriggerLastRun = new Map();
    ...
    router.patch('/jobs/:slug', (req, res) => {
        ...
        try {
            ...
            if (fs.existsSync(jobsFile)) {
                ...
                const jobEntry = (jobsData.jobs || jobsData).find((j) => j.slug === slug);
                if (jobEntry) {
    ...
    router.get('/jobs/events', (req, res) => {
        ...
        if (ctx.scheduler) {
            for (const job of ctx.scheduler.getJobs()) {
                ...
                if (st)
                    lastStates.set(job.slug, JSON.stringify(st));
            }
        }
    ...
    router.get('/jobs/history', (_req, res) => {
        ...
        const history = ctx.scheduler.getRunHistory();
        const sinceHours = parseInt(_req.query.sinceHours) || undefined;
        const result = _req.query.result;
        ...
        const resultFilter = result && validResults.includes(result) ? result : undefined;
    ...
    router.get('/jobs/:slug/history', (req, res) => {
        ...
        const sinceHours = parseInt(req.query.sinceHours) || undefined;
    ...
    router.get('/skip-ledger', (_req, res) => {
        ...
        const sinceHours = parseInt(_req.query.sinceHours) || 24;
        const slug = _req.query.slug;
        if (slug && !JOB_SLUG_RE.test(slug)) {
            res.status(400).json({ error: 'Invalid job slug' });
            return;
    ...
    router.post('/imessage/reply/:recipient', (req, res) => {
        ...
        if (ctx.sessionManager && 'clearIMessageInjectionTracker' in ctx.sessionManager) {
            ctx.sessionManager.clearIMessageInjectionTracker(recipient);
        }
    ...
    router.get('/relationships', (req, res) => {
        ...
        const sortBy = VALID_SORTS.includes(rawSort)
            ? rawSort
            : 'significance';
    ...
    router.post('/relationships/import', (req, res) => {
        ...
        for (const rec of records) {
            ...
            if (existing) {
                ...
                if (rec.notes) {
                    ctx.relationships.updateNotes(record.id, rec.notes);
                }
    ...
    router.patch('/updates/config', (req, res) => {
        ...
        if (ctx.autoUpdater) {
            ctx.autoUpdater.config.autoApply = autoApply;
        }
    ...
    router.post('/dispatches/:id/evaluate', (req, res) => {
        ...
        const success = ctx.dispatches.evaluate(req.params.id, decision, reason);
        if (!success) {
            res.status(404).json({ error: 'Dispatch not found' });
            return;
        }
    ...
    router.post('/secrets/drop/:token', (req, res) => {
        ...
        const submission = secretDrop.submit(req.params.token, _csrf, values);
        if (!submission) {
        ...
        if (submission.topicId && ctx.sessionManager) {
            ...
            if (targetSession && ctx.sessionManager.isSessionAlive(targetSession)) {
                ...
                ctx.sessionManager.injectPasteNotification(targetSession, `<system-reminder>${systemMsg}</system-reminder>`);
            }
            else {
    ...
    router.get('/events', (req, res) => {
        ...
        const rawSinceHours = parseInt(req.query.since, 10) || 24;
    ...
    router.post('/hooks/plan-prompt', async (req, res) => {
        ...
        const sessions = ctx.sessionManager.listRunningSessions();
        const session = sessions.find(s => s.id === instar_sid || s.claudeSessionId === session_id);
        if (session) {
    ...
    router.get('/serendipity/stats', (_req, res) => {
        ...
        const countJsonFiles = (dir) => {
            try {
                return fs.readdirSync(dir).filter((f) => f.endsWith('.json') && !f.endsWith('.tmp')).length;
            }
            catch {
                return 0;
            }
        ...
        try {
            const files = fs.readdirSync(serendipityDir).filter((f) => f.endsWith('.json') && !f.endsWith('.tmp'));
    ...
    router.get('/intent/journal', async (req, res) => {
        try {
            ...
            const jobSlug = req.query.jobSlug;
    ...
    router.get('/intent/drift', async (req, res) => {
        try {
            ...
            const windowDays = req.query.window ? parseInt(req.query.window, 10) : 14;
    ...
    function buildCapabilityTelemetry(id) {
        const stats = {};
        ...
        try {
            switch (id) {
                ...
                case 'health-monitoring': {
                    const cr = safeGet(() => ctx.coherenceMonitor?.getLastReport?.());
                    const sr = safeGet(() => ctx.systemReviewer?.getHealthStatus?.());
                    const mp = safeGet(() => ctx.memoryMonitor?.getState?.());
                    const op = safeGet(() => ctx.orphanReaper?.getLastReport?.());
                    const parts = [];
                    if (cr) {
                ...
                case 'safety-trust': {
                    stats.gatesActive = true;
                    metric = 'All gates active';
                    break;
                }
                case 'coherence': {
                    const cr = safeGet(() => ctx.coherenceMonitor?.getHealth?.());
                    if (cr?.lastCheck)
                        lastActivity = cr.lastCheck;
                ...
                case 'scheduled-jobs': {
                    ...
                    if (js) {
                        ...
                        if (js.activeJobSessions > 0)
                            parts.push(`${js.activeJobSessions} running`);
                        if (js.queueLength > 0)
                            parts.push(`${js.queueLength} queued`);
                        metric = parts.join(' · ');
                    }
                    break;
                }
                case 'quota': {
                    ...
                    if (qs) {
                        ...
                        if (qs.lastUpdated)
                            lastActivity = qs.lastUpdated;
                        const parts = [`Weekly usage ${Math.round(qs.usagePercent)}%`];
                        if (qs.fiveHourPercent != null)
                            parts.push(`5h rate ${Math.round(qs.fiveHourPercent)}%`);
                        metric = parts.join(' · ');
                    }
                    break;
                }
                case 'telegram': {
                    ...
                    if (ts) {
                        ...
                        if (ts.topicMappings > 0)
                            parts.push(`${ts.topicMappings} topics`);
                        metric = parts.join(' · ') || 'Connected';
                    }
                    if (ls) {
                        stats.totalMessages = ls.totalMessages ?? 0;
                    }
                    break;
                }
                case 'whatsapp': {
                ...
                case 'message-routing': {
                    metric = 'Routing active';
                    break;
                }
                case 'memory': {
                    metric = 'Context assembly active';
                    break;
                }
                case 'evolution': {
                    const ed = safeGet(() => ctx.evolution?.getDashboard?.());
                    if (ed) {
                        ...
                        const parts = [];
                        if (ed.evolution?.totalProposals)
                            parts.push(`${ed.evolution.totalProposals} proposals`);
                        if (ed.gaps?.totalGaps)
                            parts.push(`${ed.gaps.totalGaps} gaps`);
                        if (ed.learnings?.applied)
                            parts.push(`${ed.learnings.applied} applied`);
    ...
    router.get('/systems/status', (_req, res) => {
        ...
        const health = errorCount > 0 ? 'error' : 'healthy';
    ...
    router.post('/trust/grant', (req, res) => {
        ...
        const event = ctx.adaptiveTrust.grantTrust(service, operation, level, statement);
        res.json(event);
    });
    // GET /trust/elevations — pending elevation suggestions
    ...
    router.post('/autonomy/profile', (req, res) => {
        ...
        const validProfiles = ['cautious', 'supervised', 'collaborative', 'autonomous'];
    ...
    router.patch('/identity/soul', (req, res) => {
        ...
        try {
            const result = ctx.soulManager.patch({ section, operation, content, source }, trustLevel);
    ...
    router.post('/telemetry/enable', (_req, res) => {
        ...
        try {
            ...
            if (!fileConfig.monitoring)
                fileConfig.monitoring = {};
            if (!fileConfig.monitoring.telemetry)
                fileConfig.monitoring.telemetry = {};
    ...
    router.post('/commitments', (req, res) => {
        ...
        const { type, userRequest, agentResponse, topicId, source, configPath, configExpectedValue, behavioralRule, expiresAt, verificationMethod, verificationPath } = req.body;
    ...
    router.get('/episodes/stats', (req, res) => {
        if (!ctx.activitySentinel) {
            res.status(503).json({ error: 'Activity sentinel not enabled' });
            return;
        }
    ...
    router.get('/episodes/sessions/:sessionId', (req, res) => {
        ...
        if (!synthesis) {
            res.status(404).json({ error: 'Session synthesis not found' });
            return;
        }
        res.json(synthesis);
    });
    ...
    router.post('/messages/send', async (req, res) => {
        ...
        try {
            ...
            const result = await ctx.messageRouter.send(from, to, type, priority, subject, body, options);
    ...
    router.get('/messages/inbox', async (req, res) => {
        ...
        try {
            ...
            if (req.query.unread === 'true')
                filter.unread = true;
            if (req.query.fromAgent)
                filter.fromAgent = req.query.fromAgent;
            if (req.query.threadId)
                filter.threadId = req.query.threadId;
            if (req.query.limit)
                filter.limit = Math.min(parseInt(req.query.limit, 10) || 50, 200);
            if (req.query.offset)
    ...
    router.get('/messages/threads', async (req, res) => {
        ...
        try {
            ...
            const threads = await ctx.messageRouter.listThreads(status);
    ...
    router.post('/messages/spawn-request', async (req, res) => {
        ...
        try {
            ...
            if (!result.approved) {
                ctx.spawnManager.handleDenial({ requester, target, reason, context, priority, suggestedModel, suggestedMaxDuration, pendingMessages }, result);
            }
    ...
    router.get('/messages/route-score', async (req, res) => {
        ...
        try {
            ...
            const scores = ctx.summarySentinel.findBestSession(subject, body, ctx.config.projectName);
    ...
    if (ctx.handshakeManager) {
        const threadlineRoutes = createThreadlineRoutes(ctx.handshakeManager, ctx.threadlineRouter, {
                            ...
                            if (targetToken) {
                                ...
                                const envelope = {
                                    ...
                                    delivery: {
                                        phase: 'received',
                                        transitions: [
                                            { from: 'created', to: 'received', at: now, reason: 'local delivery' },
                                        ],
                                        attempts: 1,
                                    },
                                };
    ...
    function checkReviewRateLimit(sessionId, maxPerMinute) {
        const now = Date.now();
    ...
    router.get('/review/history', (_req, res) => {
        ...
        const history = ctx.responseReviewGate.getReviewHistory({
            sessionId: _req.query.sessionId,
            reviewer: _req.query.reviewer,
            verdict: _req.query.verdict,
            since: _req.query.since,
            recipientId: _req.query.recipientId,
            limit: _req.query.limit ? parseInt(_req.query.limit, 10) : undefined,
        });
    ...
    router.delete('/review/history', (_req, res) => {
        ...
        const sessionId = _req.query.sessionId;
        if (!sessionId) {
    ...
    router.get('/review/stats', (_req, res) => {
        ...
        res.json(ctx.responseReviewGate.getReviewerStats({
            period: _req.query.period || undefined,
            since: _req.query.since,
        }));
    });
    // ── Coherence Proposals ─────────────────────────────────────────
    ...
    router.put('/prompt-gate/topic/:topicId/override', (req, res) => {
        ...
        try {
            ...
            if (!data.topicOverrides)
                data.topicOverrides = {};
            const overrides = data.topicOverrides;
            const existing = (overrides[String(topicId)] ?? {});
            if (autoApproveAll !== undefined)
                existing.autoApproveAll = autoApproveAll;
            if (relayAll !== undefined)
                existing.relayAll = relayAll;
    ...
    router.get('/features', (req, res) => {
        ...
        if (stateFilter) {
            const states = stateFilter.split(',').map(s => s.trim());
    ...
    router.get('/features/digest', (req, res) => {
        ...
        const userId = req.query.userId || 'default';
        const thresholdDays = req.query.thresholdDays ? parseInt(req.query.thresholdDays, 10) : 15;
        res.json({
    ...
    router.post('/features/evaluate-context', async (req, res) => {
        ...
        const { topicCategory, conversationIntent, problemCategories, autonomyProfile, enabledFeatures, userId, } = req.body || {};

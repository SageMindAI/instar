...
async function spawnSessionForTopic(sessionManager, telegram, sessionName, topicId, latestMessage, topicMemory, userProfile) {
    ...
    if (contextContent) {
        ...
        parts.push(inlineContext, ``, `IMPORTANT: Your response MUST acknowledge and continue the conversation above. Do NOT introduce yourself or ask "how can I help" — the user has been talking to you. Pick up where the conversation left off.`, ``, `The user's latest message:`, `[telegram:${topicId}] ${msg}`);
        bootstrapMessage = parts.join('\n');
    }
    else {
        // No conversation history — new session.
        // Still inject agent + user context if available.
        const newSessionParts = [];
        if (agentContextBlock) {
 ...
 */
function messageToPipeline(msg, topicName) {
    return {
        id: msg.id,
        sender: {
            telegramUserId: msg.metadata?.telegramUserId ?? 0,
            firstName: msg.metadata?.firstName ?? 'Unknown',
            username: msg.metadata?.username ?? undefined,
        },
        topicId: msg.metadata?.messageThreadId ?? 1,
        topicName,
        content: msg.content,
...
function wireTelegramRouting(telegram, sessionManager, quotaTracker, topicMemory, userManager, fixCommandHandler) {
    ...
    telegram.onTopicMessage = (msg) => {
        ...
        const telegramUserId = msg.metadata?.telegramUserId ?? 0;
 ...
 */
function wireWhatsAppRouting(whatsapp, sessionManager) {
...
function wireIMessageRouting(imessage, sessionManager) {
     ...
     */
    function buildIMessageInjection(sender, text, senderName, isRespawn = false) {
        ...
        lines.push(`--- iMessage SESSION (${sender}) ---`, `This is a PERSISTENT conversational session via iMessage.`, `MANDATORY: After EVERY response, relay your message back to the user:`, `  cat <<'EOF' | ${replyScript} "${sender}"`, `  Your response text here`, `  EOF`, ``, `CRITICAL: After replying, STAY AT THE PROMPT and wait for follow-up messages.`, `Do NOT exit. More messages will be injected as [imessage:${sender}] prefixed text.`, `Strip the [imessage:...] prefix before interpreting messages.`, `Only relay conversational text — not tool output or internal reasoning.`, `--- END iMessage SESSION ---`);
    ...
    imessage.onMessage(async (msg) => {
        ...
        if (targetSession && sessionManager.isSessionAlive(targetSession)) {
            ...
            try {
                console.log(`[imessage→session] Spawning session for ${imessage.constructor.maskIdentifier?.(sender) || sender}...`);
                ...
                const newSession = await sessionManager.spawnInteractiveSession(undefined, sessionName);
 ...
 */
async function ensureAgentAttentionTopic(telegram, state) {
    const existingTopicId = state.get('agent-attention-topic');
    if (existingTopicId) {
 ...
 */
async function ensureSlackAttentionChannel(slack, state) {
    const existingChannelId = state.get('slack-attention-channel');
 ...
 * Created once on first server start, persisted in state.
 */
async function ensureAgentUpdatesTopic(telegram, state) {
    const existingTopicId = state.get('agent-updates-topic');
    if (existingTopicId) {
 ...
 */
function cleanupTelegramTempFiles() {
    const tmpDir = '/tmp/instar-telegram';
    try {
...
export async function startServer(options) {
     ...
     */
    function notify(tier, category, message, topicId) {
        // Telegram: via notification batcher
        const resolvedTopicId = topicId ?? _notifyState?.get('agent-attention-topic') ?? 0;
        if (resolvedTopicId) {
     ...
     */
    function formatCoherenceFailure(checkName, message) {
        switch (checkName) {
            case 'output-sanity':
    ...
    if (options.foreground) {
        ...
        staleGuard.registerSnapshot('instar-version', startupVersion, () => {
            try {
        ...
        if (promptGateConfig?.enabled) {
            ...
            detector.on('prompt', async (prompt) => {
                try {
        ...
        if (whatsappConfig) {
            try {
                ...
                whatsappAdapter = new WhatsAppAdapter(whatsappConfig.config, config.stateDir);
                await whatsappAdapter.start();
                const waConf = whatsappConfig.config;
                const backendType = waConf.backend ?? 'baileys';
                if (backendType === 'business-api') {
                    const { BusinessApiBackend } = await import('../messaging/backends/BusinessApiBackend.js');
                    const businessApiConf = waConf.businessApi;
                    whatsappBusinessBackend = new BusinessApiBackend(whatsappAdapter, businessApiConf, {
                    ...
                    const baileysBackend = new BaileysBackend(whatsappAdapter, baileysConfig, {
                ...
                if (telegram && whatsappAdapter) {
                    ...
                    const crossAlerts = new CrossPlatformAlerts({
                        ...
                        getAlertTopicId: () => state.get('agent-attention-topic') ?? null,
                    });
                    crossAlerts.start();
        ...
        if (slackConfig) {
            try {
                ...
                slackAdapter = new SlackAdapter(slackConfig.config, config.stateDir);
        ...
        if (_topicResumeMap) {
            ...
            sessionManager.on('injectionDropped', (info) => {
        ...
        const restartWatcher = new ForegroundRestartWatcher({
            ...
            onRestartDetected: async (request) => {
                ...
                if (runningSessions.length > 0) {
                    notify('IMMEDIATE', 'system', `Applying update to v${request.targetVersion} — restarting now. Active sessions will resume automatically.`);
                }
                else {
        ...
        memoryMonitor.on('stateChange', ({ from, to, state: memState }) => {
            ...
            if (to !== 'normal') {
                const tier = to === 'critical' ? 'IMMEDIATE' : 'SUMMARY';
        ...
        sessionManager.on('sessionComplete', async (session) => {
            try {
            ...
            try {
                ...
                presenceProxy = new PresenceProxy({
                    ...
                    getProcessTree: (sessionName) => {
                        ...
                        try {
                            const panePid = shellExecSync(`tmux display-message -t '=${sessionName}' -p '#{pane_pid}' 2>/dev/null`).toString().trim();
                            if (!panePid)
                                return [];
                            const childPids = shellExecSync(`pgrep -P ${panePid} 2>/dev/null`).toString().trim();
                            if (!childPids)
                                return [];
                            const pids = childPids.split('\n').filter(Boolean).join(',');
                            const psOutput = shellExecSync(`ps -o pid=,command= -p ${pids} 2>/dev/null`).toString().trim();
                            if (!psOutput)
                                return [];
        ...
        sleepWakeDetector.on('wake', async (event) => {
            ...
            if (event.sleepDurationSeconds > 300) {
                ...
                notify('DIGEST', 'system', `Machine woke up after ${mins > 60 ? `${Math.round(mins / 60)}h` : `${mins}m`} of sleep. Everything's reconnected and running.`);
            }
        });
        ...
        const operationGate = extOpsEnabled ? new ExternalOperationGate({
            ...
            services: (extOpsConfig?.services ?? {}),
        ...
        if (sentinel && telegram) {
            telegram.onSentinelIntercept = async (text, _topicId) => {
        ...
        const tmuxOps = {
            getForegroundProcess(tmuxSession) {
                ...
                catch { /* @silent-fallback-ok — tmux query, delivery layer handles unknown */
                    return 'unknown';
                }
            },
            ...
            hasActiveHumanInput(_tmuxSession) {
                // Agent sessions don't have human input — safe to inject
                return false;
            },
            sendKeys(tmuxSession, text) {
                try {
                ...
                catch { /* @silent-fallback-ok — send-keys boolean return */
                    return false;
                }
            },
            getOutputLineCount(tmuxSession) {
                try {
                ...
                catch { /* @silent-fallback-ok — capture-pane line count, 0 triggers inline delivery */
                    return 0;
                }
            },
        };
        ...
        const summarySentinel = new SessionSummarySentinel({
            ...
            captureOutput: (tmuxSession) => {
                try {
                    ...
                    return null;
                } // @silent-fallback-ok — tmux capture-pane for sentinel
            },
        });
        ...
        if (systemReviewEnabled) {
            const alertTopicId = state.get('agent-attention-topic') ?? undefined;
        ...
        try {
            ...
            if (threadlineRelayClient) {
                ...
                const syntheticThreadIds = new Map();
                function getSyntheticThreadId(fingerprint) {
                ...
                threadlineRelayClient.on('gate-passed', async (decision) => {
                    ...
                    if (!result.handled && !msg.threadId) {
                        envelope.message.threadId = getSyntheticThreadId(senderFingerprint);
                        result = await threadlineRouter.handleInboundMessage(envelope, relayContext);
                    }
                    if (result.error)
                        console.warn(`[relay] Router error: ${result.error}`);
                    if (result.spawned)
                        console.log(`[relay] Spawned session for ${senderName} (trust: ${trustLevel}, thread: ${result.threadId})`);
                    if (result.resumed)
                        console.log(`[relay] Resumed session for ${senderName} (thread: ${result.threadId})`);
                });
        ...
        if (selfKnowledgeTree && featureRegistry) {
            selfKnowledgeTree.probes.register('feature-discovery', async () => {
                ...
                const lines = ['Feature Discovery Status:'];
            ...
            try {
                ...
                if (dashTopicId) {
                    ...
                    if (dashUrl) {
                        ...
                        const telegramConfig = config.messaging?.find((m) => m.type === 'telegram');
                        if (telegramConfig?.config) {
                            telegramConfig.config.dashboardPin = config.dashboardPin || '';
